package com.example.androidstocks.utils;
...
import com.example.androidstocks.stockquotesapp.vo.QuoteVO;
...
public class RestClient {
	...
	/*
	 * this method is not required.As we canget String using EntityUtil
	 */
	...
	public List<QuoteVO> getQuotes(String[] data) {
		...
		HttpGet httpget = new HttpGet(request);		// Execute the request
		...
		try {
			...
			if (entity != null) {
				...
				if(results.get("quote")instanceof JSONArray)//if more than one item to search
				...
				else                  //for one item to search
					{
					JSONObject tempObject=results.getJSONObject("quote");
					String open = tempObject.getString("Open");
					String close = tempObject.getString("Close");
					String low = tempObject.getString("Low");
					String volume = tempObject.getString("Volume");
